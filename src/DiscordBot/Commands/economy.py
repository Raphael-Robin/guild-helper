...
class EconomyCog(commands.Cog):
    def __init__(self, bot: commands.Bot, economy_manager: IEconomyManager, database_manager: IDatabaseManager):
        self.bot = bot
        self.economy_manager = economy_manager
        self.database_manager = database_manager

    @app_commands.command(name="balance", description="Display the balance of a user.")
    @app_commands.describe(user="The Discord user to check the balance of (defaults to yourself)")
    async def balance(self, interaction: discord.Interaction, user: discord.Member | None = None):
        ...
        if len(players) == 1:
            embed.add_field(name="Character", value=players[0].albion_character_name, inline=True)
            embed.add_field(name="Balance", value=f"{players[0].balance:,}", inline=True)
            embed.add_field(name="All-Time", value=f"{players[0].all_time_balance:,}", inline=True)
        else:
    ...
    @app_commands.command(name="remove-balance", description="[Admin] Remove balance from a user.")
    ...
    async def remove_balance_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        if isinstance(error, app_commands.MissingPermissions):
            await interaction.response.send_message(
                "❌ You don't have permission to use this command.", ephemeral=True
            )

    @app_commands.command(name="leaderboard", description="Show players with the highest current balance.")
    async def leaderboard(self, interaction: discord.Interaction):
        await interaction.response.defer()
        players = await self.economy_manager.get_players_with_highest_balance(PAGE_SIZE, offset=0)
        embed = _build_leaderboard_embed(players, page=0, alltime=False)
        view = LeaderboardView(economy_manager=self.economy_manager, page=0, alltime=False)
        await interaction.followup.send(embed=embed, view=view)

    @app_commands.command(name="leaderboard-alltime", description="Show players with the highest all-time balance.")
    async def leaderboard_alltime(self, interaction: discord.Interaction):
        await interaction.response.defer()
        players = await self.economy_manager.get_players_with_highest_alltime_balance(PAGE_SIZE, offset=0)
        embed = _build_leaderboard_embed(players, page=0, alltime=True)
        view = LeaderboardView(economy_manager=self.economy_manager, page=0, alltime=True)
        await interaction.followup.send(embed=embed, view=view)
...
class LeaderboardView(discord.ui.View):
    ...
    async def update_leaderboard(self, interaction: discord.Interaction):
        ...
        if self.alltime:
            players = await self.economy_manager.get_players_with_highest_alltime_balance(PAGE_SIZE, offset)
        else:
            players = await self.economy_manager.get_players_with_highest_balance(PAGE_SIZE, offset)
